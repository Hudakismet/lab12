...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		int colonIndex = line.indexOf(":"); //find colon separating parent and children
		if (colonIndex < 0)
			throw new TreeException("illegal line");
		String parent = line.substring(0, colonIndex); //substrings start to colon (parent name)
		String childrenString = line.substring(colonIndex + 1); //substring from colon to end (all children names)
		String[] childrenArray = childrenString.split(","); //array of children names
		...
		{
			parentNode = root.getNodeWithName(parent); //search for named node

			if(parentNode == null) //if parent node not found, throw exception
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) {
			throw new TreeException(name1 + " is not a relative");
		}
		TreeNode node2 = root.getNodeWithName(name2);	// node whose name is name2
		if (node2 == null) {
			throw new TreeException(name2 + " is not a relative");
		}
